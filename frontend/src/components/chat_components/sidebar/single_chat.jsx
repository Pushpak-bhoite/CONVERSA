...
// console.log('props.item ', props.item)// --> chat details 
// console.log('props.user ', props.user) // --> --> current user details 
export default function SingleChat(props) {
  ...
  // console.log('unseen_chats -->', unseen_chats) 
  ...
  // console.log('unseen_chats[props.item._id]------------>', unseen_chats[props.item._id])
  ...
  if (props.item) { //Used of the Optional Chaining Operator (?.) .if condition returns the value of null or undefined the it always returns the undefined 
    if (props.item.chat_type === "Personal") {
      ...
      if (props.item.users[0]?._id === props.user._id) {
        if (props.item.users[1]?.profile_img) {
          profile_img = `${process.env.REACT_APP_IPADDRESS}/uploads/` + props.item.users[1]?._id + "/" + props.item.users[1]?.profile_img
        }
        else {
        ...
        if (props.item.users[0].profile_img) {
          profile_img = `${process.env.REACT_APP_IPADDRESS}/uploads/` + props.item.users[0]?._id + "/" + props.item.users[0]?.profile_img;
        }
        else {
          profile_img = `${process.env.REACT_APP_IPADDRESS}/uploads/avatar.jpg`
        }
      ...
      let last_message_time = new Date(messages[props.item._id]?.at(-1).sentAt) //its like messages[chat Id ]
      return (
