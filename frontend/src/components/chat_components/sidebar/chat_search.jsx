...
function Chat_Search(){
  ...
  useEffect(()=>{
    const res=async()=>{
      ...
      if(result.profile_img){
        url_of_image=`${process.env.REACT_APP_IPADDRESS}/uploads/`+result._id+"/"+result.profile_img;
      }
      else{
        url_of_image=`${process.env.REACT_APP_IPADDRESS}/uploads/avatar.jpg`
      }
  ...
  async function change_profile(e){
    ...
    const result_change=await  axios.post(`${process.env.REACT_APP_IPADDRESS}/api/changeuserdetails/user_profile`,{...data},{headers: {'Content-Type': 'multipart/form-data','ngrok-skip-browser-warning': 'true'}});
  ...
  async function log_out(){
    ...
    if(user_data!==null){
       try {
         ...
         const response = await axios.get(`${process.env.REACT_APP_IPADDRESS}/api/auth/log-out`,{headers: {'Content-Type': 'multipart/form-data','ngrok-skip-browser-warning': 'true'}});
         navigate("/sign-in");
       } catch (error) {
